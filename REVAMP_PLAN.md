...
**Live field-name reconciliation (build-loop finding F3, resolved 2026-05-21).** A schema check of base `appOsvuyy5eK43QTx` / table `tblSmNrHROMLm7vOS` ("Master Deal Pipeline") found all 16 fields above already exist with the correct types. The four financial fields exist under the table's established convention — **`Revenue 2024`** (`fldfUOMF98BAk8Qeo`), **`Revenue 2025`** (`fld8Pmhi9M7m5qaUf`), **`Cash Flow 2024`** (`fldwX2NkTE2E66pln`), **`Cash Flow 2025`** (`flde6Fr88nm4BAoE1`) — matching the pre-existing `Revenue 2022/2023` and `Cash Flow 2022/2023` fields. The table rows "2025 Revenue / 2025 Cash Flow / 2024 Revenue / 2024 Cash Flow" denote those exact existing fields; **these live names are canonical** (no rename — they hold data). The overnight-search and submit-url skills MUST write to the live names. Other fields and their IDs: Listing ID `fld81k0uFwqkHaEEI`, Direct Listing URL `fldMCmSVQjYv3odok`, Listing Screenshot `fldrPuxZHGsYZuxTO`, Date Added `fldoZVwrhWaGGMlFR`, Date Updated `fld3TRpVYopXL7LLm`, Previous Asking Price `fldySRjfm1P8Nodes`, Link Health Status `fldlsuLeSFhFKQuFc`, Link Last Checked `fldMXwyQbEWPXbqE2`, Disposition `fldw0xk1YBkmP7sBD`, Lead Score `fld2ipICYNLjaDm39`, Prospect Eval Report `fld9InVXs4RqgtNDo`, Source `fldiGyXTk6Ybb6J1L`.
