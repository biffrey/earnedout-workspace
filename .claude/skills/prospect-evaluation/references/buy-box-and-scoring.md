...
| 2 | EBITDA | ≥ $1,000,000 / year | SDE is acceptable as a proxy if EBITDA is not disclosed; note the distinction. **No upper cap** — large EBITDA targets pass the header but flag deal-size fit separately in the memo. |
...
When the target is a **roll-up add-on** for an existing platform company (Applied Development for sign language/CART, or Fambro Waste Management for commercial waste), the Buy Box header still displays ✅/❌ against the **standard** thresholds above — but the report must also include a secondary "Add-on Thresholds" row showing whether the target clears the relaxed floors:

| Criterion | Standard | Add-on floor |
|---|---|---|
| Full-time employees | ≥ 10 | ≥ 5 |
| EBITDA | ≥ $1M (no upper cap) | ≥ $500K |
| Years in business | ≥ 10 | ≥ 5 |

The other three header checks (3-yr growth, asking price ≤4x, DSCR >1.4) are unchanged for add-ons.

If a target fails the standard thresholds but passes the add-on floors, the overall verdict should be **CONDITIONAL (add-on)** rather than FAIL, with a note: "Does not meet standalone thresholds but qualifies as a roll-up add-on for {{platform company}}."
...
- **Add-on thresholds** (5 FTE / $500K EBITDA / 5 yrs) apply only when the target is a roll-up add-on. Standalone targets must meet the full standard thresholds.
...
**Add-on thresholds:** ✅ employees (8 ≥ 5), ✅ EBITDA ($800K ≥ $500K), ✅ years (7 ≥ 5) → **CONDITIONAL (add-on)**.
