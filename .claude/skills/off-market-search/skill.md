...
`templates/daily-dashboard.html` — the **same Jinja2 template** the
`overnight-search` skill renders. The dashboard is **whole-tracker**, not
per-run: it must show on-market and off-market leads together.

- **Section A (`new_finds`)** — only the leads created/updated in *this*
  off-market run.
- **Section B (`running_queue`)** — **every** `tblSmNrHROMLm7vOS` record with
  `Disposition = "Active"`, regardless of run or `Source`, sorted by score
  desc. Query the full tracker for this — do **not** populate it from this
  run's own leads alone (that omits every on-market lead and every prior run).
- **Section C (`revisit_bucket`)** — **every** record with
  `Disposition = "Revisit for Roll-up"`.
- **Section D** — run-summary stats.

This mirrors `overnight-search` skill Step 10: Sections B/C always render from
the full Airtable tracker, so a dashboard generated by *either* pipeline shows
the complete pipeline. Off-market rows get the **"Off-Market" badge**
(`.chip.offmarket`, rendered when `lead.source` starts with `"Off-Market"`) in
Sections A, B, and C — no `Source` column is added, on-market rows unchanged.
See `references/airtable_write.md` §5. _(Badge: s7; wiring + whole-tracker
query: s9.)_
