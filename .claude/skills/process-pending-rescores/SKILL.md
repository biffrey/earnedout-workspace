...
> **Skill copies & sync policy.** PRIMARY: `/Users/biffreybraxton/.claude/skills/process-pending-rescores/` — make all edits here. Mirror (never edit directly; refresh via `rsync -a --delete` from the PRIMARY): `/Users/biffreybraxton/published-listing-search/.claude/skills/process-pending-rescores/`, which is tracked in the `earnedout-workspace` GitHub repo — commits there must contain only PRIMARY-synced content.
